...
MODEL_PATH = "/home/sergeo/Master-II/Recherches/Projet_M_I/Groupe B  ManagerApp/v2/manager_backend/tasks/grok/a3c_model_final.pth"  # Chemin vers le modèle A3C pré-entraîné
...
def mean_execution_time_algorithm(tasks: list, volunteers: list) -> dict:
    """
    Algorithme basé sur le délai d'exécution moyen, privilégiant les volontaires avec
    un taux de complétion élevé (indicateur de rapidité et fiabilité).
    ...
    volunteer_resources = {
        v["volunteer_id"]: {
            ...
            "available_disk": v["resources"]["disk_space_mb"] / 1024,
            "available_gpu": v["resources"]["gpu"],
            "completion_rate": v["performance"]["completion_rate"]
        }
        for v in volunteers
    }
    ...
    for task in sorted_tasks:
        ...
        # Trier par completion_rate décroissant pour privilégier les plus rapides
...
MAX_TASKS = 50
MAX_VOLUNTEERS = 600
EXPECTED_INPUT_DIM = 32  # 20 tâches + 10 volontaires + 2
EXPECTED_ACTION_DIM = 200  # 20 × 10
...
def a3c_algorithm(tasks: list, volunteers: list, model_path: str = MODEL_PATH) -> dict:
    ...
    env_tasks = [
        EnvTask(
            ...
            cpu_req=t["required_resources"].get("cpu", 0.5),  # Réduit
            mem_req=t["required_resources"].get("ram", 256) / 1024,  # MB → GB, réduit
            duration_est=1.0,
            task_type="generic"
        )
        for t in tasks
    ...
    env = VolunteerSchedulingEnv(env_workflow, env_volunteers)

    # Vérifier les dimensions
    actual_input_dim = env.observation_space['shape'][0]
    ...
    if actual_input_dim != EXPECTED_INPUT_DIM or actual_action_dim != EXPECTED_ACTION_DIM:
        logger.warning(f"Dimensions incompatibles : attendu input={EXPECTED_INPUT_DIM}, action={EXPECTED_ACTION_DIM}, "
                       f"reçu input={actual_input_dim}, action={actual_action_dim}")

    # Charger le modèle
    model = ActorCriticNet(input_dim=EXPECTED_INPUT_DIM, action_dim=EXPECTED_ACTION_DIM)
    try:
        model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
        model.eval()
        logger.info(f"Modèle A3C chargé depuis {model_path}")
    except Exception as e:
        logger.error(f"Erreur lors du chargement du modèle A3C : {e}")
        # Logique de secours
        assignments = defaultdict(list)
        for task in env_tasks:
            ...
            if valid_volunteers:
                ...
                v_id = next(v["volunteer_id"] for v in volunteers if v["username"] == vol.name)
                t_id = task.id
                t_name = task.name
                assignments[v_id].append((t_id, t_name))
                logger.debug(f"Tâche {t_id} assignée au volontaire {v_id} (secours)")
            else:
                logger.warning(f"Aucun volontaire valide pour la tâche {task.id} (secours)")
        logger.info(f"Assignations de secours terminées. Tâches assignées : {len(assignments)}/{len(tasks)}")
        return dict(assignments)
    ...
    while not done and step < max_steps:
        ...
        with torch.no_grad():
            ...
            if policy_logits.size(-1) != EXPECTED_ACTION_DIM:
                logger.error(f"Dimension de policy_logits incorrecte : {policy_logits.size(-1)} au lieu de {EXPECTED_ACTION_DIM}")
                break
            mask = torch.zeros(EXPECTED_ACTION_DIM)
            for idx in valid_actions:
                if idx < EXPECTED_ACTION_DIM:
                    mask[idx] = 1
        ...
        for v_name, task_names in info["task_assignments"].items():
            try:
                # Extraire l'ID du volontaire depuis v_name (format: Volunteer_{volunteer_id})
                ...
                for task_name in task_names:
                    # Extraire l'ID de la tâche depuis task_name (format: Task_{task_id})
