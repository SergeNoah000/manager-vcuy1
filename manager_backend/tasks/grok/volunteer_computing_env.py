...
class Volunteer:
    def __init__(self, id, cpu, mem, disk, uplink, downlink, reliability, vol_type):
        ...
        self.cpu_used = 0  # Non utilisé
        self.mem_used = 0  # Non utilisé
        self.disk_used = 0  # Non utilisé
...
class VolunteerSchedulingEnv:
    def __init__(self, workflow, volunteers, w1=1000.0, w2=10.0, w3=0.001, w4=200.0):
        ...
        self.max_tasks = 20
        self.max_volunteers = 10
        self.observation_space = {'shape': (self.max_tasks + self.max_volunteers + 2,)}
        self.action_space = {'n': self.max_tasks * self.max_volunteers}
    ...
    def _get_state(self):
        ...
        task_states += [0.0] * (self.max_tasks - len(task_states))
        volunteer_states = [v.cpu / (v.cpu or 1) for v in self.volunteers.values()]
        volunteer_states += [0.0] * (self.max_volunteers - len(volunteer_states))
    ...
    def _get_valid_actions(self):
        valid_actions = []
        for t_idx, task_id in enumerate(self.available_task_ids):
            ...
            for v_idx, vol_id in enumerate(self.vol_ids):
                ...
                if (vol.cpu >= task.cpu_req and
                    ...
                    action = t_idx * len(self.vol_ids) + v_idx
        ...
        if not valid_actions:
            ...
            for t in self.workflow.tasks.values():
                logger.debug(f"Tâche {t.id}: cpu_req={t.cpu_req}, mem_req={t.mem_req}, "
                             f"size_in={t.size_in}, size_out={t.size_out}")
            for v in self.volunteers.values():
                logger.debug(f"Volontaire {v.id}: cpu={v.cpu}, mem={v.mem}, disk={v.disk}")
        return valid_actions

    def step(self, action):
        reward = 0.0
        max_assign = len(self.task_ids) * len(self.vol_ids)

        if action < max_assign and self.available_task_ids:
            t_idx = action // len(self.vol_ids)
            v_idx = action % len(self.vol_ids)
